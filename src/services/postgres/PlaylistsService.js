...
const {mapDBToPlaylistSong, mapDBToPlaylistActivity} = require('../../utils');

class PlaylistsService {
    constructor(songsService, activityService, collaborationsService) {
        this._pool = new Pool();
    ...
    async getActivity(playlistId) {
        ...
        return mapDBToPlaylistActivity(playlist[0].id, rows);
    }
    ...
    async addPlaylistSong(playlistId, songId, userId) {
        const songIdDB = await this._songsService.verifySongInDatabase(songId);
